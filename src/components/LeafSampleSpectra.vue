<template>
  <b-container>
    ...
    <b-card
      ...
      v-show="showSampleSpectra"
    >
      <b-card-header header-bg-variant="dark" header-text-variant="light">
        <b-form-group fluid v-slot="{ ariaDescribedby }" :label="leaf">
    ...
    this.$store.subscribe((mutation, state) => {
      switch (mutation.type) {
        case "save_sample_spectra":
          var self = this;
          if (
            (self.whichSpectra !== "main-spectra") &
            (typeof self.whichSpectra !== undefined)
          ) {
  ...
  methods: {
    ...
    updateChart(event) {
      ...
      if (this.which == "reflectance" || this.which == "both") {
        ...
        d3.selectAll(".sample-spectra-graph .spectra_r").attr(
          "transform",
          event.transform
        );
      }
      if (this.which == "transmittance" || this.which == "both") {
        ...
        d3.selectAll(".sample-spectra-graph .spectra_t").attr(
          "transform",
          event.transform
        );
      }
