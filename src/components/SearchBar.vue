<template>
  <b-container>
    <b-card no-body class="tab-group" v-show="showSearch">
      <b-tabs card class="search-tabs">
        <b-tab :title="search_by_species" active>
          <b-input-group size="lg" class="search-group">
            ...
            <b-input-group-append>
              <b-button
                variant="primary"
                class="clear-button btn btn-danger"
                v-on:click="clear"
                v-if="searchNotEmpty"
                >x</b-button
              >
        ...
        <template v-slot:tabs-end>
          <b-nav-item href="#" role="presentation">
            <b-dropdown
              class="species_list_dropdown"
              id="dropdown-left"
              variant="link"
              text="_"
              role="menu"
          ...
          <b-nav-item href="#" role="presentation" @click="dateModal"
            >{{ $t("filter_by") }} : {{ $t("date")
            }}<b-icon icon="check" v-show="dateFilterActive"></b-icon
          ></b-nav-item>
          <b-nav-item href="#" role="presentation" @click="geographyModal"
            >{{ $t("geography")
            }}<b-icon
              icon="check"
              aria-hidden="true"
              v-show="geoFilterActive"
            ></b-icon
          ></b-nav-item>
          <b-nav-item href="#" role="presentation" @click="projectModal"
            >{{ $t("project")
            }}<b-icon
              icon="check"
              aria-hidden="true"
              v-show="projectsFilterActive"
            ></b-icon
          ></b-nav-item>
        </template>
      </b-tabs>
    ...
    >
      ...
      <div>
        <b-button variant="danger" @click="resetDates">Reset filter</b-button>
      </div>
    </b-modal>
    <b-modal
      id="geography-filter"
      class="test-modal"
      :title="geo_filter_title"
      ok-only
      @shown="geoModalShown"
      @hide="geoModalHidden"
    >
    ...
    <b-modal
      id="project-filter"
      class="test-modal"
      :title="projects_filter_title"
      ok-only
      @shown="projectsModalShown"
      @hide="projectsModalHidden"
    >
      <b-container>
        <b-form-group>
          <b-form-checkbox
...
export default {
  ...
  computed: {
    ...
    placeholder() {
      if (
        (this.$store.state.search_box.projects_selected.length !== 0) |
  ...
  methods: {
    ...
    speciesDropdown() {
      this.$store.commit("get_species_list");
    },
    speciesDropdownSelected(el) {
      this.$refs.autocomplete.setValue({
        name: el.target.text,
        code: el.target.text,
      });
      this.$store.state.search_box.search_value = el.target.text;
      this.searchValue = el.target.text;
      this.search();
    },
    dateModalHidden() {
    ...
    geoModalHidden() {
      ...
      this.$store.state.search_box.geomFilter = JSON.stringify(
...
.species_list_dropdown ul,
.species_list_dropdown li,
.species_list_dropdown a {
  z-index: 999;
}

.species_list_dropdown button {
  padding: 0;
}

.species_list_dropdown button::after {
  right: 20px;
  margin-right: -20px;
}
